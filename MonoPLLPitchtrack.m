...
hold off;
timeVectorOut = zeros(1, ceil(length(F0) / (fs/100)));
pitchOut = zeros(1, ceil(length(F0) / (fs/100)));
%probOut = zeros(1, ceil(length(medTruePitch) / 441));
for i = 1:fs/100:length(F0)
    timeVectorOut (((i-1)/(fs/100)) + 1) = (i-1) / fs; 
    pitchOut (((i-1)/(fs/100)) + 1) = F0(i);
